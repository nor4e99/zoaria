...
export const petsApi = {
  ...
  getBreeds: (speciesId?: number) => api.get(`/pets/breeds/${speciesId ? `?species=${speciesId}` : ''}`),
};

export const vetsApi = {
  getAll: (params?: any) => api.get('/vets/', { params }),
  get: (id: number) => api.get(`/vets/${id}/`),
  getProfile: () => api.get('/vets/profile/'),
  updateProfile: (data: any) => api.patch('/vets/profile/', data),
  getReviews: (vetId: number) => api.get(`/vets/${vetId}/reviews/`),
  addReview: (vetId: number, data: any) => api.post(`/vets/${vetId}/reviews/`, data),
};

export const healthApi = {
  getRecords: (petId: number) => api.get(`/health/records/?pet=${petId}`),
  createRecord: (data: any) => api.post('/health/records/', data),
  getPrescriptions: (petId: number) => api.get(`/health/prescriptions/?pet=${petId}`),
  createPrescription: (data: any) => api.post('/health/prescriptions/', data),
};

export const feedingApi = {
  getLogs: (petId: number) => api.get(`/feeding/logs/?pet=${petId}`),
  createLog: (data: any) => api.post('/feeding/logs/', data),
  getGuidelines: (speciesId?: number) => api.get(`/feeding/guidelines/${speciesId ? `?species=${speciesId}` : ''}`),
};

export const activityApi = {
  getLogs: (petId: number) => api.get(`/activity/logs/?pet=${petId}`),
...
export const chatApi = {
  ...
  createConversation: (vetId: number) => api.post('/chat/conversations/', { vet: vetId }),
...
export const hubApi = {
  getArticles: (params?: any) => api.get('/hub/articles/', { params }),
  getArticle: (id: number) => api.get(`/hub/articles/${id}/`),
  getVideos: (params?: any) => api.get('/hub/videos/', { params }),
};

export const adminApi = {
  getUsers: () => api.get('/admin-panel/users/'),
  getVets: () => api.get('/admin-panel/vets/'),
